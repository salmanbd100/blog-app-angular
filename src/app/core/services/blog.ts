...
export class BlogService {
  ...
  constructor() {
    this.seedBlogs(); // ✅ run once when service is created
  }

  // ✅ 1. Seed demo blogs if storage is empty
  private seedBlogs() {
  ...
  // ✅ 3. Get one blog by ID
  getBlogById(id: number): Blog | undefined {
    const blogs = this.getAllBlogs();
    return blogs.find(b => b.id === id);
  }

  // ✅ 4. Add a new blog (for later when we want "create post")
  addBlog(blog: Blog) {
    const blogs = this.getAllBlogs();
    blogs.push(blog);
